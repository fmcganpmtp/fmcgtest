    ...
    public function ProuctRquestSearch(Request $request)
    {
        ...
        elseif (!is_null($main_category_id)) { 
            ...
            $products = ProductRequest::
                //leftJoin("users","users.id", "=", "product_requests.user_id")
                select(["product_requests.product_description AS prd_name", "product_requests.*",])
                ->where("product_requests.parent_id", "<>", $user_id)
                ->whereDate('req_end_date', '>', Carbon::now())
  ...
  public function MyProuctRquestSearch(Request $request)
    {
        ...
        $today = Carbon::now();
            $products = ProductRequest::leftJoin("users","users.id", "=", "product_requests.user_id")
                ->select(["product_requests.product_description AS prd_name","product_requests.country_id AS country_id", "product_requests.*",])
                ->where("product_requests.parent_id", $user_id)
    ...
    public function addNewSeller(Request $request)
    {
        ...
        $usertype = Auth::guard('user')->user()->usertype;  
        if($usertype=='seller')
            echo json_encode("Co-Seller Added");
        else
            echo json_encode("Co-Buyer Added");
