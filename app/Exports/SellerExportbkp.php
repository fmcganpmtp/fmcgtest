...
class SellerExport implements FromCollection,ShouldAutoSize,WithHeadings
{
    ...
    public function collection()
    {
       ...
       $records=$records ->where('usertype','seller') ->where('seller_type','Master')->where('users.status','<>','Deleted')     
        ...
        DB::raw("CONCAT(buyer_companies.company_street, ',', buyer_companies.company_location, ',', buyer_companies.company_zip) as address")
        ,DB::raw("countries.name as country_name"),DB::raw("(SELECT expairy_date FROM subscriptions WHERE subscriptions.user_id = users.id order BY subscriptions.id DESC limit 1) as expairy_date"),DB::raw("'' AS ctgry"),DB::raw("'' AS cmpny_type"),
