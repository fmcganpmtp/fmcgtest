...
@if(!empty($avgStar))  <a href="#reviews" class="rating-count">{{ $productReviewCounts->Starcount }} Reviews</a> 
...
@endif
          ...
          @if($is_active_subscriber  && $usertype!="guest")
                   ...
                   echo $currency.'&nbsp;'.number_format($product->product_price,3); ?> 
           ...
           <span class="in-stock">@if($product->unlimited_stock!='Unlimited')<?=$stock_status?>@endif ( <?php echo $product->unlimited_stock=='Unlimited'? 'Unlimited Stock' : ( $product->stock_count=='' ? '' : $product->stock_count+0) ;?> )</span></div> 
          @endif
...
@endif
                ...
                @if((!$is_active_subscriber ||$usertype=="guest") && ($logged_id != $seller->id ) )
       ...
       // e.preventDefault();
        ...
        $.ajax({
          ...
          success: function (data) { 
            $('#responsedata').html(data.success);
              $('#productForm').trigger("reset");
              setTimeout(function() {$('#exampleModal').modal('hide');}, 12000);
              table.draw();
                                  ...
                                  $("#divreviews").append(string);
                                   ...
                                   var skipcount=parseInt($("#start_from").val())+ initial ;
                                    if(dataItems.count!=skipcount)
                                      $("#divreviews").append('<br><div class="default-btn" style="width:15%;margin:0 auto" id="loadmore" onclick="loadmore()">Load More</div>');
