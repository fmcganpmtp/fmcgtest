            ...
            <div class="card-header">Company infromations: </div>
              <div class="card-body">
                ...
                <div class="form-group">
                  ...
                  <?php 
                  $cmp_type ='';
                  if(!is_null(Auth::guard('user')->user()->BuyerCompany->company_type))
                  $cmp_type = Auth::guard('user')->user()->BuyerCompany->company_type;
                  ?>
                  @if(!empty($company_types))
                  @foreach ($company_types as $company_type)
                  @if(Auth::guard('user')->user()->BuyerCompany->company_type==$company_type->id ) {{$company_type->company_type??''}}@endif
                       @endforeach 
                  @endif</span>
                </div>
			</div>
...
?>
         ...
         ?>
              ...
              <div class="col-lg-8 col-12 ck-bx9">
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" id="flexSwitchCheckChecked"  name="newsletter_status" value="Yes"  @if( Auth::guard('user')-> user()->newsletter_status=="Yes") checked @endif >
                  <label class="form-check-label" for="flexSwitchCheckChecked">Newsletter Emails</label>
                </div>
              </div>
    ...
    <div class="card answer">
           ...
           @if(Session::has('message_not_match')) <div class="alert alert-danger">{{Session::get('message_not_match') }}</div> @endif
			<form action="{{ route('reset.user.password.post') }}" method="POST">
                @csrf
            <div class="row">
