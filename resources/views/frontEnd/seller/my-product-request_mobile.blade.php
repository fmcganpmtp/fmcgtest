...
@section('title', 'My Product Request')
@section('content')
...
.pr-req .mar-btm-20 .default-btn{width:100%; line-height:20px; font-size:13px;    padding: 5px 10px 11px;    text-transform: uppercase;

}

.no-brdd{border:none;}
.autocomplete{background:none!important;}

.sg-list input{background:none;}

.crt-rwq{padding:18px 10px!important;}
...
.req-msg-btn a{color:#fff;}


.product-title h3{margin-bottom:10px!important;}
.edit-link{background:#cd6d20 !important;padding:4px 9px !important; color:#fff !important; position:absolute !important;right:0 !important; top:0 !important;}
</style>
...
<section class="products-area ptb-54 innerpageC pr-req">  
  <div class="container">
      ...
      <div class="col-lg-3">
          ...
          @else
          <a href="javascript:void(0)" onclick="showerrorPrdReq()" class="default-btn crt-rwq">Create Product request</a>
         @endif
      </div>
...
function product_search(){
  ...
  $fmcg.ajax({
                            type : 'post',
                            url:"{{route('MyProuctRquestSearch')}}",
                            async: false,
                            data:{
                              _token:'{{ csrf_token() }}',
                            ...
                            success:function(data)
                            {
                                  ...
                                  let dataItems = JSON.parse(data);
                                   ...
                                   dataItems.product_data.map((item) => {
                                    initial++;
                                    productflag=true;
                                    var app_path="<?php echo url('edit-product-request/')?>";
                                    
                                  
                                   var string= '<div class="col-lg-4 col-12 wow fadeInUp pr-list">';
                                   string+= '<div class="product-thumbnail hovereffect"><div class="days-remain">'+item.days_remaining+' Days Remaining</div><div class="pro-img"><img src="'+item.product_thumbnail;
                                   string+='"></div><div class="product-title"><h5 class="looking-for"><b>We are Looking for:</b></h5> <h3>';
                                   ...
                                   string+='<h3>'+item.company_name+'</h3>';
                                   }
                                    @endif
                                   ...
                                   //string+=country_list+'</h5></div>';
                                   if(item.country!=''){
                                   string+='&nbsp;&nbsp;<i class="fa fa-map-marker" aria-hidden="true"></i>'+item.country+'</h5></div>';
                                   }
                                   string+='<div class="req-btns"><a href="javascript:void(0)" onClick="deleteProductReq('+ item.pr_id+' )" class="default-btn delete-pr">';
                                   string+='<i class="fa fa-trash-o" aria-hidden="true"></i>Delete</a>';
                                   string+='<a href="javascript:void(0)" class="default-btn" onclick="ExtendRequest('+ item.pr_id+')" ><i class="fa fa-file-text-o" aria-hidden="true"></i>Extend Request </a></div>'
                                   
                                   
                                   string+='<div class="product-ms-over">';
                                   string+='<div class="pr-hover-border-bx"><div class="req-contet">';
                                   if (item.product_name.length > 20) 
                                        var prd_name = item.product_name.slice(0, 20) + "...";
                                   else
                                        var prd_name = item.product_name;
				                   string+='<h4>We are looking for:</h4>';
				                   string+='<div class="ms-over-txt">';
				                   
					               string+='<p>'+prd_name+'</p></div><h5>';
					               if(item.quantity!=''){
                                   ...
                                   string+='<a  href="'+app_path+'/'+item.pr_id+'" class="edit-link"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a></div></div>';
...
/*$fmcg('#category_id_l').on("change", function (e) { 
   ...
   $fmcg('#category_id_l').change(function(){
         ...
         var app_path="<?php echo url('my-product-requests/')?>";
         if(cat_id === '')
...
function deleteProductReq(data){
    var id=data;           
    var deleteurl="{{route('deleteProductRequest', ':id')}}".replace(':id', '')+data;
...
function ExtendRequest(data){
    var id=data;           
    var extendurl="{{route('ExtendRequest', ':id')}}".replace(':id', '')+data;
