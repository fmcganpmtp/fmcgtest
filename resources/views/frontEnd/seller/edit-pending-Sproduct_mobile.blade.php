         ...
         <div class="card prd_diplicate">
            ...
            <form id="formsubmit">
             <div class="row">
                     ...
                     <span id="txtname"></span> </div>
                         ...
                         @if(empty($product->currency_id))
                                 <div class=" col-lg-2 col-12">
                                    <div class="form-group">
                                       <label >Currency: <?php echo $product->currency; ?></label>
                                    </div>
                                 </div>
                ...
                <div class=" col-lg-2  col-12">
                  <div class="form-group">
                    <label>Price: <sup id="not_man"  @if(strtolower(trim(old('price_on_request',$product->price_on_request))) == strtolower('Price on request')) style="display:none;" @endif >*</sup></label>
                ...
                <div class=" col-lg-4 col-12 price_request_outer">
                  <div class="form-group">
                    ...
                    <div class="form-field-style"form-field-style" >
                      <input class="price_request" type="checkbox" name="price_on_request" @if(strtolower(trim(old('price_on_request',$product->price_on_request)))==strtolower('Price on request')) checked @endif    value="Price on request"> {{ __('Price on Request') }} </div>
                  </div>
                </div>
...
<div class=" col-lg-4 col-12 " >
 ...
 <div class=" col-lg-4 col-12">
                             ...
                             <input type="checkbox"    class="choose_existing" name="exist_category"  value="eisting_cat"> {{ __('Choose from existing categories.') }}
                           </div> 
@endif 
<div class="row default_cats" @if(!empty($cat_selected)) style="display:inline-flex;" @else style="display:none;" @endif>
                
<?php  
$imageExtensions = ['jpg', 'jpeg', 'gif', 'png', 'bmp', 'svg', 'svgz', 'cgm', 'djv', 'djvu', 'ico', 'ief','jpe', 'pbm', 'pgm', 'pnm', 'ppm', 'ras', 'rgb', 'tif', 'tiff', 'wbmp', 'xbm', 'xpm', 'xwd'];        
$count=0; $executed1='no'; $executed2='no';  $prev_sub=[]; ?>
...
@endif  
                            ...
                            @if($product_image->thumbnail=="no" || $product_image->thumbnail=="yes")
                                @if(filter_var($product_image->image_path, FILTER_VALIDATE_URL) === FALSE)
                                    ...
                                    <div class="thumplist45">
                                        <div id="image-block{{ $product_image->id }}"> <a href="javascript:void(0)" onClick="removeImage({{ $product_image->id }},'block')" class=""><span class="red_round remove-input-field"><i class="fa fa-minus-circle" aria-hidden="true"></i></span></a> <img src="{{$product_image->image_path}}" class="img-responsive" alt="" srcset=""> </div>
                                    </div>
                                    @else
                                    <div class="thumplist45">
     ...
     <!--                          @php $selectedCountries =[];if(!empty($product->country_ids))$selectedCountries = explode(",",$product->country_ids);@endphp-->
     ...
     <!--               @php if(!empty($product->variants))$selectedvariants = explode(",",$product->variants);else $selectedvariants =[]; @endphp-->
...
//var $fmcg = $.noConflict();
$fmcg(".choose_existing").click(function() {
        
      if($fmcg(this).prop("checked") == true){
	  $fmcg(".default_cats").show(); }
     else {
       $fmcg(".default_cats").hide();
    }
    }); 
...
$fmcg(document).ready(function () {
    ...
    $fmcg("#formsubmit").on('submit', function(e) {
            ...
            formData.append('_token', "{{ csrf_token() }}"); 
                ...
                $fmcg.ajax({
                type: "post",
                url: "{{ route('update.PendingSellerProduct') }}",
                data: formData,
                ...
                error: function (xhr) {
                    ...
                    $fmcg("#txtname").empty().append("<p style='color:red'>"+errors.errors.name[0]+"</p>");
                      ...
                      $fmcg("#txtname").empty();
                    var cattextlen=$fmcg('#categories').length;
                    var catflg=0;
                    if(errors.errors.categories)
                        $fmcg(".dldcategory").empty().append("<p style='color:red'>"+errors.errors.categories[0]+"</p>");
                      else
                        $fmcg(".dldcategory").empty(); 
                      if(errors.errors.categories && cattextlen===0)
                       {
                           catflg=1;
                           $fmcg(".dldcategory1").empty().append("<p style='color:red'>"+errors.errors.categories[0]+"</p>");
                       }  
                       else
                       {
                            catflg=0;
                            $fmcg(".dldcategory1").empty(); 
                       }
                        
                    if(errors.errors.category_id)
                        $fmcg(".dldcategory1").empty().append("<p style='color:red'>"+errors.errors.category_id[0]+"</p>");
                    else if(catflg===0)
                        $fmcg(".dldcategory1").empty(); 
...
$fmcg(".choose_existing_currency").click(function() {
           
         if($fmcg(this).prop("checked") == true){
   	  $fmcg(".default_currencies").show(); }
                     else {
                            $fmcg(".default_currencies").hide();
                       }
                   }); 	
