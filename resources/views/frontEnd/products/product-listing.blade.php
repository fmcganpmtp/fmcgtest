...
$fmcg('input[name="priceOnRequest"]').click(function(){
        var $radio = $fmcg(this);
        $fmcg('#start_from').val(0);
        $fmcg("#product_list").empty();   
        ...
        $radio.siblings('input[name="priceOnRequest"]').data('waschecked', false);
    }); 
...
function fnPushPriceFilter(checkbox){ 
  ...
  $fmcg('#start_from').val(0);
  $fmcg("#product_list").empty();    
    if($fmcg('.rad1').is(':checked') || $fmcg('.rad2').is(':checked')) { 
        priceOnRequest=$fmcg('input[name="priceOnRequest"]:checked').val();
        product_search();
